...
export function BentoGridExampleThree() {
  return (
    <section className="w-full -mt-40 md:-mt-44 lg:-mt-48">
      <Container className="relative z-10 flex w-full flex-col overflow-hidden pb-20 md:pb-30 lg:pb-30">
      <Header>
        <h2
          className="-tracking-xl mx-auto w-fit text-center text-2xl leading-8 font-medium text-black">
          Brand Identity
        </h2>
      </Header>
      <p
        className="mx-auto mt-4 max-w-lg text-center text-base text-neutral-600">
        Logo, colours, typography, and the visual foundation your business
        needs to look consistent everywhere — online, in print, and across your
        marketing materials.
      </p>
      <div className="cols-1 relative z-10 mt-8 grid gap-6 md:auto-rows-[440px] md:grid-cols-5">
        ...
        <Card className="flex flex-col justify-between md:col-span-2">
          <CardContent className="h-40">
            <CardTitle>First Impressions Matter</CardTitle>
            <CardDescription>
              A professional, unified look builds trust from the first
              interaction — whether someone lands on your website, sees your
              truck, or picks up a brochure.
            </CardDescription>
          </CardContent>
          <CardSkeletonBody>
            <div
              className="mt-2 ml-6 h-full w-full rounded-lg border border-neutral-200 bg-neutral-100 p-4 dark:border-neutral-700 dark:bg-neutral-800">
              <img
                src="/dashboard.webp"
                alt="Dashboard"
                width={500}
        ...
        <Card className="flex flex-col justify-between md:col-span-3">
          <CardContent className="h-40">
            <CardTitle>Pricing and Timeline</CardTitle>
            <CardDescription>
...
const includesItems = [
  "Logo design and file delivery",
  "Colour palette with usage guidance",
  "Typography selection and pairing",
  "Basic brand asset package",
  "Files ready for web and print use",
];

const IncludesList = () => {
  return (
    <div className="w-full">
      <h3 className="text-base font-medium text-black">Includes</h3>
      <div className="relative mt-6 flex flex-col gap-4 pl-7">
        <IncludesDashedConnector itemCount={includesItems.length} />
        {includesItems.map((label) => (
          <p
            key={label}
            className="relative z-10 flex w-fit items-center gap-1 rounded-full bg-natural-white px-2 py-0.5 text-xs text-neutral-700 shadow-sm ring-1 shadow-black/5 ring-neutral-200"
          >
            <SiteIcon name="palette" size={16} className="text-heading" />
            {label}
          </p>
        ))}
      </div>
...
const IncludesDashedConnector = ({ itemCount }: { itemCount: number }) => {
  const x = 8;
  const r = 8;
  const endX = 28;
  const itemHeight = 20;
...
export const SkeletonOne = () => {
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center">
      ...
      <div
        className="relative z-30 mx-auto grid w-full max-w-lg grid-cols-1 gap-4 p-8 [perspective:1000px] [transform-style:preserve-3d] sm:p-0 md:grid-cols-3">
        <Container
          initial={{ y: 0 }}
...
const CardTitle = ({
  ...
  return (
    <h3 className={cn("text-base font-medium text-black", className)}>
      {children}
    </h3>
  );
};
const CardDescription = ({
  ...
  return (
    <p className={cn("mt-4 text-base text-neutral-600", className)}>
      {children}
    </p>
  );
};
...
const Card = ({
  ...
  return (
    <motion.div
      whileHover="animate"
      className={cn(
        "group isolate flex flex-col overflow-hidden rounded-3xl bg-natural-white",
        className
      )}>
      {children}
    </motion.div>
